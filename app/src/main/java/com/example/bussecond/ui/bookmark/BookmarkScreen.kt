...
fun MyTryItem(stopEtaList: List<StopEta>,
    ...
    Row(modifier = Modifier.wrapContentSize()) {
        Column {
            Text(text = "往${stopEtaList[0].dest_tc}")
            Text(text = stopEtaList[0].route)
            Text(text = bookmark.stopname)
        }
        Column {
            for(etaData in stopEtaList){
                Row {
                    val eta: String = if(etaData.eta == null) {"-"} else if (etaData.eta == "-") {"-"} else { etaData.eta.substring(11,19)}
                    Text(text = eta)
                }
            }
        }
